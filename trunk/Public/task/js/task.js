...
$(function () {
    ...
    */
    ...
    $(document).on('pageInit', '#step', function (e, id, page) {
        ...
        var STEP = null;
        var task_id = getParam('task_id', 0), apply_id = getParam('apply_id', 0), step_id, curr_step, total_step, type;
        page.off('click');
        var boxTop = $('.wrap-step-inner section'), boxBottomInner = $('.wrap-form-inner'), boxBottom = $('.step-form'),
        text = $('.text-form'), file = $('.file-form'), label = file.find('label'), arrow = $('.fold-step-form');
        var submitObj = {
            applyId: apply_id,
            stepResult: [{
                taskId: task_id,
        ...
        getStepDetail(task_id, apply_id, function () {
            var currStep = STEP['taskStep'][curr_step - 1];
        ...
        page.on('click', '.btn-submit', function (event) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.code) {
                        ...
                        setTimeout(function () {
                            $.router.load(CON_PATH + '/step?task_id=' + task_id + '&apply_id=' + apply_id + '&curr_step=' + curr_step);
                        }, 1000);
                    } else {
                        submitArray.pop();
                    }
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.code) {
                        ...
                        $('.step-count span').text(curr_step + '/' + total_step);
                        callback();
                    }
                },
                error: server_error
