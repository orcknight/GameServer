...
class ApplyController extends BaseController {
    ...
    public function job() {
        ...
        //申请人数已经满了，不允许申请了
        if($this->isJobNumberFull($jobId)){
            
            $data['code']  = 0;         
            $data['msg']  = "报名人数已满，请申请别的兼职！";  
            $this->ajaxReturn($data);    
        }
    ...
    * @param mixed $jobId
    */
    private function isJobNumberFull($jobId){
        
        //获取工作所需人数
        $number = 0;
        $querySql = "SELECT number FROM job WHERE id = $jobId";
        $result = M()->query($querySql);
        if($result){
        ...
        $querySql = "SELECT COUNT(*) AS count FROM job_apply WHERE jobId = $jobId AND uStatus = $status";
        $result = M()->query($querySql);
        $this->trackLog("result", $result);
        if(!$result){
