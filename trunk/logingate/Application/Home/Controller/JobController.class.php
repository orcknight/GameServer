...
class JobController extends BaseController {
	...
	public function jobs() {
		...
		if($type==JobEnum::SEARCH_TYPE_HOT) {
            ...
            $this->ajaxReturn($jobService->getIndexPageJobs($args));
	...
	public function getJobDetails() {
        ...
        //返回申请人数和通过人数
        $this->getAppliedNumber($job['id'], $job);
    ...
    * @param mixed $jobId
    */
    private function getAppliedNumber($id, &$job){
        
        $appliedCount = 0;
        $postedCount = 0;
        
        //申请人数
        $status = JobEnum::USTATUS_APPLYING . ',' . JobEnum::USTATUS_FINISHING;
        $querySql = "SELECT COUNT(*) AS count FROM job_apply WHERE jobId = $id AND uStatus IN($status)";
        $result = M()->query($querySql);
        if($result){
            
            $appliedCount = $result[0]['count'];
        }
        
        //通过人数
        $status = JobEnum::USTATUS_FINISHING;
        $querySql = "SELECT COUNT(*) AS count FROM job_apply WHERE jobId = $id AND uStatus = $status";
        $result = M()->query($querySql);
        if($result){
            
            $postedCount = $result[0]['count'];
        }
        
        $job['appliedCount'] = $appliedCount;
        $job['postedCount'] = $postedCount;
    }
}