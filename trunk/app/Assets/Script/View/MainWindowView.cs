...
public class MainWindowView : BaseView {
	...
	public void OnStream(SocketIOEvent socketIoEvent){
		...
		for (int i = 0; i < jlist.Count; ++i) {  //遍历JArray  
			...
			case ProtoCode.TASK_LIST_CODE:
				LoadTaskList (msg);
				break;
            case ProtoCode.COMBAT_ENTER_CODE:
                EnterCombatMode(msg);
                break;
	...
	void OnSwipe( SwipeGesture gesture ) 
	{
		...
		if (gesture.Direction == FingerGestures.SwipeDirection.Left) {
			...
			if (Mathf.Abs (move.x) > 100) {
                ...
                PopWindowManager popWindowManager = this.transform.GetComponent<PopWindowManager>();
                m_TaskBar = popWindowManager.OpenByName("taskbar");
            }
		} else if (gesture.Direction == FingerGestures.SwipeDirection.Right) {

            PopWindowManager popWindowManager = this.transform.GetComponent<PopWindowManager>();
            popWindowManager.closeByName("taskbar");
            return;
	...
	public void OnBottomBarClick(GameObject obj){
		...
		if (obj.name.Equals ("BtnTaskList")) {
			...
			jsonObject.AddField ("data", sendData);
			m_Socket.Emit ("unity_stream", jsonObject);
		}else if(obj.name.Equals("BtnPlayerAttribute")) {

            //发送tasklist消息
            JSONObject sendData = new JSONObject();
            JSONObject jsonObject = new JSONObject();
            sendData.AddField("uuid", "000000");
            jsonObject.AddField("cmd", "hp");
            jsonObject.AddField("data", sendData);
            m_Socket.Emit("unity_stream", jsonObject);
        }
	}
	...
	private void PopObjectInfoWindow(string msg){
		...
		PopWindowManager popWindowManager = this.transform.GetComponent<PopWindowManager> ();
		popWindowManager.OpenByName ("popdialog", msg);
		return;
	...
	private void OpenBag(string msg){
		...
		PopWindowManager popWindowManager = this.transform.GetComponent<PopWindowManager> ();
		popWindowManager.OpenByName ("bag", msg);
	...
	private void EnterCombatMode(string msg){

        Singleton<ContextManager>.Instance.Push(new CombatContext());
        return;

        PopWindowManager popWindowManager = this.transform.GetComponent<PopWindowManager> ();
		popWindowManager.OpenByName ("combat", msg);
	}
